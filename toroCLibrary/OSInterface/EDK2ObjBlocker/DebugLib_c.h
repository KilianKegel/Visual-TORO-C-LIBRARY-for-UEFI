...
#ifndef _DEBUG_LIB_C_H_
#define _DEBUG_LIB_C_H_
  ...
  )
{
    ...
    DebugVPrint(ErrorLevel, Format, ap);
  ...
  )
{
    static ROMPARM_VWXPRINTF RomParm = { \
        ...
        /*fAjustDifference      */ 0 ,\
        /*fWide                 */ 0 ,\
        /*fUEFIFormat           */ 1 /*0 != (MOFINE_UEFIFMTSTR & fTraceEn),\*/
    };

    CDE_APP_IF* pCdeAppIf = __cdeGetAppIf();

    if (__cdeTianocoreDEBUGGet(pCdeAppIf))
    {
        if (NULL != pCdeAppIf->pCdeServices)
            pCdeAppIf->pCdeServices->pVwxPrintf(
                pCdeAppIf,          // this
                &RomParm,           // IN ROMPARM_VWXPRINTF *pRomParm,
                Format,             // IN const void *pszFormat,
                pCdeAppIf->pCdeServices->pPutDebug, //void (*pfnDevPutChar)(UINT16/*wchar_t*/c,void** ppDest/*address of pDest*/),
                pCdeAppIf,          // CDE_APP_IF *CdeAppIf parm for pPutDebug == _CdeDbgPutChar()
                (unsigned)-1,       // unsigned dwCount,
                VaListMarker        // IN va_list ap
            );
    }
    return;
}
