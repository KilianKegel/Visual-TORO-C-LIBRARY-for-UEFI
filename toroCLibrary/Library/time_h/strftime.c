...
Synopsis
    #include <time.h>
    size_t strftime(char *s, size_t maxsize,const char * format,const struct tm * ptm);
...
**/
size_t strftime(char* pDest, size_t maxsize, const char* format, const struct tm* ptm)
{
    size_t nRet = 0;
        ...
        for (n = f = fErr = fFinished = 0, state = PROCESS_WRITECHARS; n < (int)maxsize && fErr == 0 && fFinished == 0; ) {
            ...
            case PROCESS_FORMAT:
                ...
                case 'c':   f++; sprintf(buffer, "%s %s %2d %02d:%02d:%02d %04d", wday_name_short[ptm->tm_wday], mon_name_short[ptm->tm_mon], ptm->tm_mday, ptm->tm_hour, ptm->tm_min, ptm->tm_sec, 1900 + ptm->tm_year); break;
                    //                        case 'c':   f++;sprintf(buffer,"%02d/%02d/%02d %02d:%02d:%02d",1 + ptm->tm_mon,ptm->tm_mday,ptm->tm_year,ptm->tm_hour,ptm->tm_min,ptm->tm_sec);break;
                case 'd':   f++; sprintf(buffer, "%02d", ptm->tm_mday); break;
                case 'H':   f++; sprintf(buffer, "%02d", ptm->tm_hour); break;
                case 'I':   f++; sprintf(buffer, "%02d", ptm->tm_hour % 12); break;
                case 'j':   f++; sprintf(buffer, "%03d", 1 + ptm->tm_yday); break;
                case 'm':   f++; sprintf(buffer, "%02d", ptm->tm_mon); break;
                ...
                case 'x':   f++; sprintf(buffer, "%02d/%02d/%02d", 1 + ptm->tm_mon, ptm->tm_mday, ptm->tm_year); break;
            ...
            case PROCESS_COPYBUFFER:
                ...
                if (l + n < (int)maxsize) {
                    memcpy(&pDest[n], buffer, (size_t)l);
                    n += (int)l;
                    state = PROCESS_WRITECHARS;
                }
                else {
